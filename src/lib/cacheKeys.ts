...
export const CACHE_KEYS = {
  ...
  exportSetting: {
    ...
    paged: (userId: string, page: number, pageSize: number, freeText?: string, hasTotalColumn?: boolean) => ['exportSetting', userId, 'paged', String(page), String(pageSize), freeText ?? '', String(hasTotalColumn ?? '')],
    count: (userId: string, freeText?: string, hasTotalColumn?: boolean) => ['exportSetting', userId, 'count', freeText ?? '', String(hasTotalColumn ?? '')],
  ...
  connection: {
    ...
    paged: (userId: string, page: number, pageSize: number, freeText?: string, type?: string, reportType?: string) => ['connection', userId, 'paged', String(page), String(pageSize), freeText ?? '', type ?? '', reportType ?? ''],
    count: (userId: string, freeText?: string, type?: string, reportType?: string) => ['connection', userId, 'count', freeText ?? '', type ?? '', reportType ?? ''],
